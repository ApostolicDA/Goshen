    ...
    def build_report(self):
        ...
        def comment_cards():
            ...
            return "".join(
                ...
                f"<span class='commenter'>{e.get('username','unknown')}</span>"
                f"<span class='comment-time'>{e['event_ts'][11:16]}</span>"
        ...
        reconnection_note = (
            ...
            f"WARNING: {stats['reconnections']} stream interruption(s) detected - "
...
<style>
  :root {{ --gold:#c9a84c; --gold2:#f4a261; --dark:#0d0d0d; --card:#1a1a1a; --border:#2a2a2a; --text:#e8e0d0; --muted:#888; }}
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  body {{ background:var(--dark); color:var(--text); font-family:monospace; font-size:13px; line-height:1.7; }}
  .header {{ background:#0d0d0d; border-bottom:1px solid var(--gold); padding:48px 40px 36px; }}
  .church-name {{ font-size:11px; letter-spacing:.35em; text-transform:uppercase; color:var(--gold); margin-bottom:8px; }}
  .report-title {{ font-size:38px; font-weight:300; line-height:1.1; margin-bottom:16px; }}
  .report-title strong {{ color:var(--gold); }}
  .meta-row {{ display:flex; gap:32px; flex-wrap:wrap; color:var(--muted); font-size:11px; }}
  .meta-row span {{ color:var(--text); }}
  .container {{ max-width:900px; margin:0 auto; padding:40px; }}
  .section-title {{ font-size:11px; letter-spacing:.3em; text-transform:uppercase; color:var(--gold); margin:40px 0 16px; }}
...
<div class="header">
  ...
  <div class="meta-row">
    ...
    <div>Session &nbsp;<span style="font-size:10px">{stats['session_id'][:12]}...</span></div>
  </div>
</div>
<div class="container">
  {reconnection_note}
...
new Chart(document.getElementById('viewerChart').getContext('2d'), {{
  ...
  options: {{
    ...
    async def save_and_email_report(self):
        ...
        subject = f"Goshen Global - TikTok Live - {stats['session_date']} - {stats['peak_viewers']} peak viewers"
        plain = (
...
def build_client(session):
    ...
    async def on_like(event):
        ...
        log(f"Like from: {event.user.nickname if event.user else user}")
        await session.add(
            "like",
    ...
    async def on_comment(event):
        ...
        log(f"Comment from {event.user.nickname if event.user else user}: {comment}")
        await session.add(
            "comment",
            user_id=str(event.user.id) if event.user else None,
            username=user,
    ...
    async def on_gift(event):
        ...
        log(f"Gift from {user}: {gift} x{repeats} ({diamonds} diamonds)")
        await session.add(
            "gift",
            user_id=str(event.user.id) if event.user else None,
            username=user,
    ...
    try:
        while True:
            ...
            try:
                ...
                log(f"Connection lost: {type(e).__name__}: {e} - reconnecting in {jitter}s...")
